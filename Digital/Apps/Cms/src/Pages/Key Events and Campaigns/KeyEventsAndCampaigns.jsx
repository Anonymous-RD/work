...
import { fetchEventsAndCompaigns, deleteEventsAndCompaign } from "../../api/api";
import { useHeader } from "../../components/context/HeaderContext";

const KeyEventsAndCampaigns = () => {
  ...
  const handleView = (event) => {
    Swal.fire({
      ...
      html: `
        ...
        <p><strong>Status:</strong> ${event.status ? "Published" : "Draft"}</p>
        <p><strong>Theme:</strong> ${event.theme}</p>
        <p><strong>Publish Date:</strong> ${event.publishDate}</p>
      `,
  ...
  const handleEdit = (event) => {
    navigate("/editkeyevents", { state: { id: event.id } }); // navigate to the edit page
  };
  ...
  const handleDelete = async (eventId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await deleteEventsAndCompaign(eventId);
          if (response.message === "Event deleted successfully") {
            setData((prevData) => prevData.filter((event) => event.id !== eventId));
            Swal.fire("Deleted!", "The event has been deleted.", "success");
          } else {
            Swal.fire("Error", response.message || "Failed to delete the event. Please try again.", "error");
          }
        } catch (error) {
  ...
  useEffect(() => {
    setTitle("Key Events And Events");
    const fetchData = async () => {
      try {
        const response = await fetchEventsAndCompaigns(); // Fetch data from the API
        const transformedData = response.data.map((event) => ({
          id: event._id,
          title: event.title,
          status: event.status === "Published", // Convert status to boolean
          theme: event.themes.map((theme) => theme.name).join(", "), // Combine theme names
          publishDate: new Date(event.createdAt).toLocaleDateString(), // Format date
        }));
    ...
    <div className="p-4 pt-20 mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div className="relative w-1/3">
          <input
            type="text"
            placeholder="Search for Events & Campaigns"
            value={searchTerm}
...
export default KeyEventsAndCampaigns;