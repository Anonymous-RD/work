  ...
  async ({ email, password }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        "https://us-central1-styletrends-5dc20.cloudfunctions.net/auth/login",
        {
          email,
          UID:res?.user?.uid
        }
      ...
      if (response.data.qr_code) {
        return {
          ...
          setup_key:response.data.setup_key,
        };
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(registerUser.fulfilled, (state, action) => {
        ...
        state.setup_key = action.payload.setup_key || null;
