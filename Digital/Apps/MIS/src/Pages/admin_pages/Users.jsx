  ...
  const applyFilters = () => {
    return users?.filter((user) => {
      ...
      const matchesFilter = filter
        ? user.role === filter || (user.mfaVerified?"Approved":"Rejected") === filter
        : true;
      ...
      <div className="flex justify-between items-center mb-4 pb-4 border-b border-[#F5F5F5] pr-7">
        ...
        <div className="flex gap-5 items-center ">
          ...
          <button
            ...
            className="text-nowrap  flex items-center gap-1 bg-[#003765] text-[white] font-semibold rounded-[10px] hover:bg-orange-300 px-4 py-6 w-[110px] h-[20px] sm:w-[138px] sm:h-[42px] text-[14px]"
            onClick={inviteHandler}
          >
            <svg width="14" height="14" className="mr-2 mb-[1px]" viewBox="0 0 14 14" fill="white" xmlns="http://www.w3.org/2000/svg">
<path d="M14 8H8V14H6V8H0V6H6V0H8V6H14V8Z" fill="white"/>
</svg>
 Invite User
          </button>
          
           <button ref={buttonRef} className="hover:text-gray-800 w-[120px] pl-4 pr-3 relative flex border border-[#F5F5F5] items-center gap-2 font-[500] py-3 rounded-[10px] text-[14px]" onClick={() =>
...
<path d="M2.83301 4.16668H11.1663V5.83334H2.83301V4.16668ZM0.333008 0.833344H13.6663V2.50001H0.333008V0.833344ZM5.33301 7.50001H8.66634V9.16668H5.33301V7.50001Z" fill="#1B212D"/>
              ...
              {openFilter && <div ref={dropdownRef} className={`rounded-xl bg-white absolute right-0 top-12 w-full text-start text-xs border border-[#F5F5F5] transition-all duration-500 delay-100 z-[20]`}>
                ...
                <div className="p-2" onClick={(e)=>setFilter(e.target.innerHTML)}>Approved</div>
                <div className="p-2" onClick={(e)=>setFilter(e.target.innerHTML)}>Rejected</div>
              </div>}
            </button>
        </div>
      </div>
