...
const TopBar = ({ icon, onIconClick }) => {
  ...
  const handleGoBack = () => {
    if (previousUrl) {
      navigate(previousUrl); // Navigate to the previous URL
    } else {
      navigate("/"); // If no previous URL, go to home page
    }
  };
  useEffect(() => {
    ...
    const afterAdmin = path.split("/admin/")[1]?.split("/")[0];
  ...
  return (
    <header className="flex flex-wrap gap-10 justify-between items-center w-full mr-3 max-md:max-w-full p-2 mt-6 bg-white ">
      <div className="flex justify-around self-stretch my-auto text-3xl font-semibold ml-8">
        {icon && (
          <>
            <button onClick={handleGoBack} className="text-[#1B212D] ml-6">
              <span onClick={onIconClick}>
                <IoArrowBackOutline />{" "}
              </span>
            </button>
            <h1 className="text-3xl font-[25px] text-gray-900 ml-6">
              {pageData}
            </h1>
          </>
        )}
