...
const SidebarItem = ({
  ...
  return (
    <div>
      ...
      <div
        onClick={handleClick}
        className={`flex gap-3 items-center py-3.5 pl-4 w-full whitespace-nowrap rounded-lg cursor-pointer ${
      ...
      >
        ...
        {children &&
          (isExpanded ? (
            <FaCaretUp className="ml-auto" fontSize={14} />
          ) : (
            <IoMdArrowDropdown className="ml-auto" fontSize={20} />
          ))}
      </div>
      {/* Child items */}
      {children && isExpanded && (
        <div className="ml-8 mt-2">
          {children.map((child, index) => (
            <NavLink
              ...
              to={child.path}
              className={({ isActive }) =>
                `flex gap-3 items-center py-2 pr-10 pl-4 w-full whitespace-nowrap ${
                  isActive
...
const Sidebar = () => {
  ...
  return (
    <nav className="flex flex-col justify-center px-6 pt-28 pb-36 bg-neutral-50 max-w-[280px]  h-[745px]">
      {/* Header */}
