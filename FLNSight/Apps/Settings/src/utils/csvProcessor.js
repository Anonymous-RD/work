export const processCSV = (csvText, requiredFields) => {
  ...
  // Original headers without lowercasing
  const headers = rows[0]
    .split(",")
    .map((header) => header.trim());

  // Perform case-insensitive matching for required fields
  const headersLower = headers.map((header) => header.toLowerCase());
  const missingFields = requiredFields
    .map((field) => field.toLowerCase())
    .filter((field) => !headersLower.includes(field));
  ...
  const dataRows = rows.slice(1).map((row, rowIndex) => {
    ...
    columns.forEach((col, index) => {
      const header = headers[index]; // Use original case headers here
      try {
