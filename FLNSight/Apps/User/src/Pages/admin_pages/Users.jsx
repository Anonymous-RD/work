...
const UsersPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      const {data}=dispatch(fetchAllUsers(token)); // Dispatch to fetch all users
      console.log(data)
  ...
  const applyFilters = () => {
    return users?.filter((user) => {
      const matchesUsername = filterData.username
        ? user.name.toLowerCase().includes(filterData.username.toLowerCase())
        : true;
      ...
      const matchesPhone = filterData.phone
        ? user?.mobile?.toString().includes(filterData.phone.toString())
        : true;
  
      const matchesRole = filterData.role
        ? user.role.toLowerCase() === filterData.role.toLowerCase()
        : true;
  
        const matchesStatus = filterData.status && filterData.status.toLowerCase() !== "all"
        ? (user.mfaVerified ? "Accepted" : "Pending").toLowerCase() === filterData.status.toLowerCase()
        : true; // If status is "All" or blank, show all.
  
      return matchesUsername && matchesEmail && matchesPhone && matchesRole && matchesStatus;
    });
  };
  ...
  //       ? user.role === filter || (user.mfaVerified?"Accepted":"Pending") === filter
  //       : true;
  ...
  const handleDelete = async (_id) => {
    try {
      const response = await dispatch(deleteUser({ id:_id, token }));
...
<path d="M14 8H8V14H6V8H0V6H6V0H8V6H14V8Z" fill="black"/>
 ...
 Invite User
           ...
           <button ref={buttonRef} className="hover:text-gray-800  pl-3 pr-3 relative flex border border-[#F5F5F5] items-center gap-2 font-[500] py-3 rounded-[10px] text-[14px]" onClick={() =>
      setOpenFilter(!openFilter)
      
    }>
           <svg className="mb-[2px]" width="16" height="12" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M2.83301 4.16668H11.1663V5.83334H2.83301V4.16668ZM0.333008 0.833344H13.6663V2.50001H0.333008V0.833344ZM5.33301 7.50001H8.66634V9.16668H5.33301V7.50001Z" fill="#1B212D"/>
</svg>

              {/* {openFilter && <div ref={dropdownRef} className={`rounded-xl bg-white absolute right-0 top-12 w-full text-start text-xs border border-[#F5F5F5] transition-all duration-500 delay-100 z-[20]`}>
                <div className="border-b p-2 border-[#F5F5F5]" onClick={(e)=>setFilter("")}>All</div>
                <div className="p-2" onClick={(e)=>setFilter(e.target.innerHTML)}>Accepted</div>
                <div className="p-2" onClick={(e)=>setFilter(e.target.innerHTML)}>Pending</div>
              </div>} */}
            </button>
        </div>
      </div>
{openFilter && <Filters query={setFilterData} setOpen={setOpenFilter}/>}
      {/* Users Table */}
    ...
    <thead className="sticky top-0 bg-white z-10">
      <tr className="text-left text-sm text-[#929EAE] uppercase">
        <th className="font-[600] text-[12px] px-4 py-4">User Name</th>
        <th className="font-[600] text-[12px] px-4 py-2">Email</th>
        <th className="font-[600] text-[12px] px-4 py-2">Phone No.</th>
        <th className="font-[600] text-[12px] px-4 py-2">Role</th>
        <th className="font-[600] text-[12px] px-4 py-2">Status of Approval</th>
      ...
      {filteredUsers?.length > 0 ? (
        filteredUsers.map((user) => (
          <tr key={user.id} className="border-b border-[#F5F5F5] last:border-b-0 text-sm">
            <td className="px-4 py-7 font-[500] text-[#1B212D]">{user.name}</td>
            <td className="px-4 py-2 text-[#78778B] font-[500]">{user.email}</td>
            <td className="px-4 py-2 text-[#78778B] font-[500]">{user.mobile}</td>
            <td className="px-4 py-2">
              <span className="bg-[#D9FFE9] text-[#27AE60] px-4 py-2 rounded text-sm">
                {user.role}
              </span>
            </td>
            <td className="px-4 py-2 ">
              <span
                className={`px-3 text-sm py-2 rounded ${
                  user.mfaVerified === true
                    ...
                    ? "bg-[#FFF1E5] text-[#F2994A]"
                    : "bg-[#FFF1E5] text-[#F2994A]"
                }`}
              >
                {user.mfaVerified?"Accepted":"Pending"}
              </span>
            </td>
        ...
        setSelectedUserID(user._id)
              ...
              <svg width="5" height="20" viewBox="0 0 5 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M4.5444 2.1941C4.5444 3.4053 3.52736 4.38822 2.2722 4.38822C1.01704 4.38822 0 3.4053 0 2.1941C0 0.981998 1.01704 -7.62939e-06 2.2722 -7.62939e-06C3.52736 -7.62939e-06 4.5444 0.981998 4.5444 2.1941ZM4.5444 9.96499C4.5444 11.1689 3.52736 12.1446 2.2722 12.1446C1.01704 12.1446 0 11.1689 0 9.96499C0 8.76104 1.01704 7.78538 2.2722 7.78538C3.52736 7.78538 4.5444 8.76104 4.5444 9.96499ZM4.5444 17.7359C4.5444 18.948 3.52736 19.93 2.2722 19.93C1.01704 19.93 0 18.948 0 17.7359C0 16.5247 1.01704 15.5418 2.2722 15.5418C3.52736 15.5418 4.5444 16.5247 4.5444 17.7359Z" fill="#A2A2A8"/></svg>
                {user._id==selectedUserID && <div ref={dropdownRef} className={`${user._id==selectedUserID?"opacity-100":"opacity-0"} rounded-xl bg-white absolute right-7 text-start w-[100px] text-xs box-shadow transition-all duration-500 delay-100 `}>
                  <div className="border-b p-2 border-[#F5F5F5]" onClick={()=>{navigate('/edit');dispatch(setSelectedUser({_id:user._id,name:user.name,email:user.email,role:user.role,mobile:user.mobile}))}}>Edit</div>
                  <div className="p-2" onClick={()=>setOpenDelete(true)}>Delete
